...
const SafetyScoreCard = ({ safetyScore, safetyStats }) => {
  ...
  const getIconSrcByLabel = (label) => {
    const iconMap = {
      [t("safety_label_no_helmet")]: "no_helmet",
      [t("safety_label_no_vest")]: "no_vest",
      [t("safety_label_no_glass")]: "no_glass",
      [t("safety_label_no_gloves")]: "no_glove",
    };
    return icons[iconMap[label]] || null;
  };

  const Exchanger = (data) => {
    if(data>0) return "+" + data+"%";
    else if(data<0) return + data+"%";
    else return "0%";
  }
  return (
    <div className="card safety-score-card">
      <div className="card-body">
        <div className="header-section">
          ...
          <span className="score-change">{Exchanger(safetyScore)}</span>
        </div>
