...
const Camera = () => {
  ...
  const alertCounts = useMemo(() => {
    ...
    const counts = alerts.reduce(
      (acc, alert) => {
        ...
        else if (alertType === "no_glass") acc.no_glass += 1;
        else if (alertType === "no_glove") acc.no_glove += 1;
        else if (alertType === "no_vest") acc.no_vest += 1;
        else acc.unknown += 1;
        return acc;
      },
      { noHelmet: 0, no_glass: 0, no_glove: 0, no_vest: 0, unknown: 0 }
    );
  ...
  useEffect(() => {
    ...
    fetchAlerts();
    intervalId = setInterval(fetchAlerts, 20000);
  ...
  const getIconSrc = (alertType) => {
    ...
    if (type === "no_glass") return icons.no_glass;
    if (type === "no_glove") return icons.no_glove;
    if (type === "no_vest") return icons.no_vest;
            ...
            <div className="summary-card p-4 rounded text-white">
              <div className="row text-center">
                {[
                  ...
                  { label: "No Glass", icon: icons.no_glass, count: alertCounts.no_glass },
                  { label: "No Glove", icon: icons.no_glove, count: alertCounts.no_glove },
                  { label: "No Vest", icon: icons.no_vest, count: alertCounts.no_vest },
