...
const ReportPage = () => {
  ...
  {/*
    ...
    */}
    ...
    const [data, setData] = useState([]);
    useEffect(() => {
        axios.get("http://localhost:8000/api/greport")
            .then((res) => {
  ...
  const itemDate = new Date(item.date.replace(/\./g, "-")); // "2025.05.12 18:01" → Date 객체
  const start = new Date(startDate);
  const end = new Date(endDate + "T23:59:59"); // inclusive
